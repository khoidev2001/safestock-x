import { ComponentScore, EnvironmentInput, EnvironmentThresholds } from "../readiness.types";
...
export function scoreEnvironment(
  ...
  if (input.temperature !== null && input.temperature > thresholds.maxTemperature) {
    const overRatio = (input.temperature - thresholds.maxTemperature) / thresholds.maxTemperature;
    scores.push(Math.max(0, 100 - overRatio * 200));
    reasons.push(`Nhiệt độ ${input.temperature}°C vượt ngưỡng ${thresholds.maxTemperature}°C`);
  }

  if (input.humidity !== null && input.humidity > thresholds.maxHumidity) {
    const overRatio = (input.humidity - thresholds.maxHumidity) / thresholds.maxHumidity;
    scores.push(Math.max(0, 100 - overRatio * 200));
    reasons.push(`Độ ẩm ${input.humidity}% vượt ngưỡng ${thresholds.maxHumidity}%`);
  }
