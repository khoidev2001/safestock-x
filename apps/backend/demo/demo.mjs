...
function info(label, value) { console.log(`  ${C.dim}${label}:${C.reset} ${value}`); }
...
async function main() {
  ...
  const rescueLogin = await api("POST", "/api/auth/login", { email: "rescue@safestock.vn", password: "rescue123" });
  const rescueRes = await fetch(`${API}/api/inventory/export`, {
    method: "POST",
    headers: { Authorization: `Bearer ${rescueLogin.accessToken}`, "Content-Type": "application/json" },
    body: JSON.stringify({ batchId: "x", quantity: 1 }),
  });
  ok(`Phân quyền: RESCUE gọi xuất kho → HTTP ${rescueRes.status} ${rescueRes.status === 403 ? "(chặn đúng)" : ""}`);
  ...
  const incident = { incidentType: "FLOOD", affectedPeople: 120, durationHours: 48, children: 20, medicalSupportCases: 4 };
  info("Tình huống", `Ngập lụt, ${incident.affectedPeople} người cô lập ${incident.durationHours}h, ${incident.children} trẻ em, ${incident.medicalSupportCases} ca y tế`);
  ...
  console.log(`  ${C.bold}Mức đáp ứng: ${fc}${mission.fulfillment}%${C.reset} ${C.dim}(mắt xích yếu nhất)${C.reset}`);
  for (const r of mission.requirements) {
    const status = r.shortage === 0 ? `${C.green}đủ${C.reset}` : `${C.red}thiếu ${r.shortage}${C.reset}`;
    ...
    if (r.neighborSuggestion?.length) {
      const s = r.neighborSuggestion.map((n) => `${n.name} ${n.distanceKm}km(${n.available})`).join(", ");
  ...
  await new Promise((res) => socket.on("connect", () => socket.emit("join", { warehouseId: wid }, res)));
  socket.on("sensor_event", (e) => {
    if (e.deviceCode.startsWith("humid")) console.log(`    ${C.magenta}[WS] cảm biến ${e.deviceCode} = ${e.value}%${C.reset}`);
  });

  console.log(`  ${C.yellow}→ Kéo độ ẩm khu y tế lên 95% (mưa bão)...${C.reset}`);
  await api("POST", "/api/simulator/events", { warehouseId: wid, deviceCode: "humid_B", eventType: "HUMID", value: 95 });
  await sleep(2000); // chờ debounce recalc + WS
  const zoneBafter = await getZoneScore(wid, "B");
  const drop = zoneB - zoneBafter;
  if (drop > 0) ok(`Điểm khu B rớt ${zoneB} → ${C.red}${zoneBafter}${C.reset} (giảm ${drop}) trong <2s`);
  ...
  const run = await api("POST", "/api/simulator/runs", { scenarioKey: "suspected_loss", warehouseId: wid, speed: 10 });
  await api("POST", `/api/simulator/runs/${run.id}/play`);
  console.log(`  ${C.yellow}→ Chạy kịch bản: loadcell giảm + cửa mở + không phiếu xuất...${C.reset}`);
  await sleep(4000);
  ...
  if (scan.detected > 0) {
    ...
    console.log(`  ${C.bold}${sc}⚠ ${inc.title} — mức ${inc.severity}, độ tin cậy ${Math.round(inc.confidence * 100)}%${C.reset}`);
    ...
    for (const e of detail.evidence) {
      console.log(`    ${C.dim}${new Date(e.occurredAt).toLocaleTimeString("vi")}${C.reset} ${e.note}`);
    }
  ...
  ok(`Hoàn trả: 15 tốt + 3 hỏng + 2 mất → phiếu ${ret.closed ? "đóng" : "mở"}, kho trừ 2 (mất thật)`);
...
main().catch((e) => {
  ...
  console.error(`${C.dim}Kiểm tra: backend :3100, ai-service :8000, Docker, đã seed chưa.${C.reset}`);
  process.exit(1);
});
