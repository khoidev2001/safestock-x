...
export function InventoryReceivingDialog({
  ...
  const submit = async () => {
    ...
    if (mode === "NEW" && [sku, name, categoryName, unit].some((value) => value.trim().length < 1)) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-black/60 p-4" role="dialog" aria-modal="true">
      <section className="my-8 w-full max-w-xl rounded-lg border bg-[var(--surface)] p-5 shadow-2xl">
        <header className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--text-muted)]">Tạo lô, ledger nhập và nhãn QR trong một giao dịch.</p>
          </div>
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          {mode === "EXISTING" ? (
            <Field label="Vật tư">
              <select className="w-full rounded-md border bg-[var(--surface)] px-3 py-2" onChange={(e) => setItemId(e.target.value)} value={itemId}>
            ...
            <>
              <Field label="SKU"><Input onChange={setSku} value={sku} /></Field>
              <Field label="Tên vật tư"><Input onChange={setName} value={name} /></Field>
              <Field label="Danh mục"><Input onChange={setCategoryName} value={categoryName} /></Field>
              <Field label="Đơn vị tính"><Input onChange={setUnit} value={unit} /></Field>
              <label className="flex items-center gap-2 text-sm">
                <input checked={consumable} onChange={(e) => setConsumable(e.target.checked)} type="checkbox" />
                Vật tư tiêu hao
              </label>
            </>
          )}
          <Field label="Kệ nhận hàng">
            <select className="w-full rounded-md border bg-[var(--surface)] px-3 py-2" onChange={(e) => setShelfId(e.target.value)} value={shelfId}>
              <option value="">Chọn kệ</option>
              {shelves.filter((shelf) => !shelf.isLocked).map((shelf) => (
                <option key={shelf.id} value={shelf.id}>{shelf.zoneLabel} / {shelf.code}</option>
              ))}
            </select>
          </Field>
          <Field label="Mã lô"><Input onChange={setBatchCode} value={batchCode} /></Field>
          <Field label="Số lượng"><Input min={1} onChange={setQuantity} type="number" value={quantity} /></Field>
          <Field label="Hạn dùng (nếu có)"><Input onChange={setExpiryDate} type="date" value={expiryDate} /></Field>
          <div className="sm:col-span-2"><Field label="Ghi chú"><Input onChange={setNote} value={note} /></Field></div>
        </div>
...
function Input({ value, onChange, type = "text", min }: { value: string; onChange: (value: string) => void; type?: string; min?: number }) {
  return <input className="w-full rounded-md border bg-transparent px-3 py-2" min={min} onChange={(e) => onChange(e.target.value)} type={type} value={value} />;
}
