...
export function InventoryQrDialog({
  ...
  const print = () => {
    ...
    popup.document.write(`<!doctype html><html><head><title>Nhãn ${escapeHtml(batch.batchCode)}</title>
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4" role="dialog" aria-modal="true">
      <section className="w-full max-w-md rounded-lg border bg-[var(--surface)] p-5 shadow-2xl">
        ...
        <div className="mt-5 grid place-items-center rounded-md bg-white p-4">
          {dataUrl ? <Image alt={`QR ${batch.item.sku}`} height={320} src={dataUrl} width={320} /> : "Đang tạo QR…"}
        </div>
        ...
        {error ? <p className="mt-3 text-sm text-[var(--color-critical)]" role="alert">{error}</p> : null}
        <button
