...
export function InventoryActionDialog({ action, batch, shelves, onClose, onSuccess }: Props) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setError(
        submitError instanceof Error ? submitError.message : "Không thực hiện được thao tác.",
      );
    } finally {
      setBusy(false);
    }
        ...
        <div className="mt-5 space-y-4">
          ...
          {action === "RECONCILE" ? (
            <label className="flex items-start gap-3 rounded-md border p-3 text-sm">
              <input
                checked={applyOverride}
                className="mt-1"
                onChange={(e) => setApplyOverride(e.target.checked)}
                type="checkbox"
              />
              <span>
                <b>Áp số đếm vào tồn hệ thống</b>
                <br />
                <span className="text-[var(--text-muted)]">
                  Bỏ chọn nếu chỉ muốn lưu biên bản kiểm kê.
                </span>
              </span>
            </label>
          ) : null}
