...
export function InventoryBulkExportDialog({
  ...
  const submit = async () => {
    ...
    try {
      ...
      setError(submitError instanceof Error ? submitError.message : "Không xuất được danh sách lô.");
    } finally {
      setBusy(false);
    }
  };
      ...
      <section className="my-8 w-full max-w-2xl rounded-lg border bg-[var(--surface)] p-5 shadow-2xl">
        <header className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--text-muted)]">Một lô lỗi sẽ rollback toàn bộ phiếu.</p>
          </div>
          ...
          {batches.map((batch) => (
            <label className="flex items-center gap-3 p-3" key={batch.id}>
              <span className="min-w-0 flex-1">
                ...
                <span className="text-xs text-[var(--text-muted)]">{batch.item.sku} · {batch.batchCode} · tồn {batch.quantity}</span>
              </span>
              <input
                ...
                max={batch.quantity}
                min={0}
                onChange={(event) => setQuantities((current) => ({ ...current, [batch.id]: event.target.value }))}
                type="number"
        ...
        <label className="mt-4 block">
          ...
          <textarea className="min-h-20 w-full rounded-md border bg-transparent px-3 py-2" onChange={(event) => setNote(event.target.value)} value={note} />
        </label>
