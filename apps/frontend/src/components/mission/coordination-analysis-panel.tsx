    ...
    <div className="mt-4 space-y-4">
      ...
      <Section title="Điều phối nội xã">
        <DataTable
          ...
          rows={analysis.coordination.allocations.map((item) => [
            item.warehouseName,
            item.sku,
            String(item.quantity),
            routeLabel(item.routeStatus, item.distanceKm, item.etaMinutes),
          ])}
      ...
      <div className="border-t pt-3 text-xs text-[var(--text-muted)]">
        ...
        <p className="mt-1">
          Snapshot {snapshot.fingerprint.slice(0, 20)}… · rule {snapshot.ruleVersion} · tính lúc{" "}
...
function EmptyState() {
  return (
    <p className="mt-4 text-sm text-[var(--text-muted)]">
      Chưa có bản tham mưu. Hãy lập bản đầu tiên để xem dữ kiện, nhu cầu và các điểm cần xác minh.
    </p>
  );
}
...
function requestId(prefix = "analysis") {
