...
import { mergeCoordinateDrafts, mergeHamletCoordinateDrafts } from "../map-marker-state";
...
test("coordinate draft only changes the matching marker", () => {
  ...
  assert.deepEqual(mergeCoordinateDrafts(markers, { b: { lat: 13.2, lng: 109.2 } }), [
    markers[0],
...
test("moving a verified hamlet invalidates verification until ADMIN verifies again", () => {
  const hamlets = [{ id: "hamlet-1", lat: 13.1, lng: 109.1, verified: true }];
