...
const KEY = process.env.NEXT_PUBLIC_MAPTILER_KEY
  ?? readEnvValue(".env.local", "NEXT_PUBLIC_MAPTILER_KEY");
if (!KEY) { console.error("Thiếu NEXT_PUBLIC_MAPTILER_KEY (đặt trong môi trường hoặc .env.local)"); process.exit(1); }
...
for (const { z, x, y } of jobs) {
  ...
  if (fs.existsSync(file) && isValidTile(fs.readFileSync(file))) { skipped++; done++; continue; }
