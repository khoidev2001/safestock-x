...
Hướng dẫn đầy đủ từ clone repository, cấu hình `.env`, chạy database/AI đến build production và cài tự khởi động trên Windows: [docs/HUONG-DAN-CAI-DAT-VA-CHAY.md](docs/HUONG-DAN-CAI-DAT-VA-CHAY.md).
...
## Tài khoản vận hành
Các mật khẩu demo mặc định phải được thay trước khi cho phép truy cập từ Internet. Không lưu mật khẩu thật trong Git, tài liệu hoặc file cấu hình dùng chung.

- ADMIN đổi mật khẩu người dùng tại mục **Tài khoản**.
- Mật khẩu mới phải có ít nhất 12 ký tự.
- Lưu mật khẩu quản trị trong trình quản lý mật khẩu riêng của đơn vị.
- Sau khi chạy lại seed trên môi trường thật, phải đổi lại toàn bộ mật khẩu trước khi mở Cloudflare Tunnel.
